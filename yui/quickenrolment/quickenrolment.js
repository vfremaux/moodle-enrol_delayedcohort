YUI.add('moodle-enrol_delayedcohort-quickenrolment', function(Y) {
    ...
    CONTROLLER.prototype = {
        initializer : function(config) {
            ...
            var base = Y.Node.create('<div class="'+CSS.PANELCONTENT+'"></div>')
                ...
                .append(Y.Node.create('<div class="'+CSS.FOOTER+'"></div>')
                    .append(Y.Node.create('<div class="'+CSS.SEARCH+'"><label for="enrolcohortsearch">'+M.str.enrol_delayedcohort.cohortsearch+':</label></div>')
            ...
            Y.all('.enrol_delayedcohort_plugin input').each(function(node){
        ...
        show : function(e) {
            ...
            var rolesselect = Y.one('#id_enrol_delayedcohort_assignable_roles');
            if (rolesselect) {
                rolesselect.focus();
            }
        },
            ...
            switch (e.type.replace(/^[^:]+:/, '')) {
                case 'cohortsloaded' :
                    ...
                    if (this.get(MORERESULTS)) {
                        var fetchmore = Y.Node.create('<div class="'+CSS.MORERESULTS+'"><a href="#">'+M.str.enrol_delayedcohort.ajaxmore+'</a></div>');
                ...
                case 'assignablerolesloaded':
                    ...
                    content = Y.Node.create('<select id="id_enrol_delayedcohort_assignable_roles"></select>');
                    for (i in roles) {
                        content.append(Y.Node.create('<option value="'+i+'">'+roles[i]+'</option>'));
                    }
                    panel.get('contentBox').one('.'+CSS.PANELROLES).setContent(Y.Node.create('<div><label for="id_enrol_delayedcohort_assignable_roles">'+M.str.role.assignroles+':</label></div>').append(content));

                    this.getDefaultCohortRole();
                    Y.one('#id_enrol_delayedcohort_assignable_roles').focus();
                    break;
    ...
    M.enrol_delayedcohort = M.enrol || {};
    M.enrol_delayedcohort.quickenrolment = {
        init : function(cfg) {
            new CONTROLLER(cfg);
        }
    }
